...
export default function CreateSessionPage() {
  ...
  useEffect(() => {
    ...
    if (s && !meta.title && moments.length === 0) {
      try {
        ...
        setPendingDraft({ meta: dm, moments: dmo });
      } catch (e) {}
    }
  }, []);
  ...
  const handleTestClass = async (momentIdx = 0, scope = "session") => {
    ...
    if (scope === "moment" && calcMomentsTotal([draftMoments[momentIdx]]) <= 0) {
      return toast.error("Ese momento necesita duración mayor que 0 para probarlo");
    }
  ...
  const recoverDraft = () => {
    ...
    setMeta(pendingDraft.meta || meta);
    setMoments(pendingDraft.moments || []);
      ...
      {pendingDraft && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/45 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-3xl border border-slate-200 bg-white p-5 shadow-2xl">
            <h2 className="text-lg font-black text-slate-900">Hay un borrador sin guardar</h2>
            <p className="mt-2 text-sm leading-relaxed text-slate-600">
              Encontramos la sesión que estabas editando. ¿Deseas continuar desde donde la dejaste?
            </p>
